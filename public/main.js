// Foursquare API Info
const foursquareKey = 'fsq3gmghQQGxVCxuroBTBW/BRySu+DPJB4+GwSL6q9vMQXM=';
const url = 'https://api.foursquare.com/v3/places/search?near=';

// OpenWeather Info
const openWeatherKey = 'b1981cf75fe18ce7f101caa3b04e7bdb';
...
const $placeDivs = [$("#place1"), $("#place2"), $("#place3"), $("#place4"), $("#place5"), $("#place6")];
...
const getPlaces = async() => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const places = [];
            for (let result in jsonResponse.results) {
                const randomIndex = Math.floor(Math.random() * jsonResponse.results.length);
                if (places.indexOf(jsonResponse.results[randomIndex]) > -1) {
                    continue;
                } else {
                    places.push(jsonResponse.results[randomIndex]);
                }
            }
            return places;
        }
